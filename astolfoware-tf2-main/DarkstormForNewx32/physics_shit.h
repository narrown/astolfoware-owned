...
public:
	...
	virtual void SetPosition(const Vector& worldPosition, const Vector& angles, bool isTeleport) = 0;
	virtual void SetPositionMatrix(const matrix3x4& matrix, bool isTeleport) = 0;
	virtual void GetPosition(Vector* worldPosition, Vector* angles) const = 0;
	...
	virtual void UpdateShadow(const Vector& targetPosition, const Vector& targetAngles, bool tempDisableGravity, float timeOffset) = 0;
	virtual int GetShadowPosition(Vector* position, Vector* angles) const = 0;
...
public:
	...
	virtual Vector			CollideGetExtent(const CPhysCollide* pCollide, const Vector& collideOrigin, const Vector& collideAngles, const Vector& direction) = 0;
	virtual void			CollideGetAABB(Vector* pMins, Vector* pMaxs, const CPhysCollide* pCollide, const Vector& collideOrigin, const Vector& collideAngles) = 0;
	...
	virtual void TraceBox(const Vector& start, const Vector& end, const Vector& mins, const Vector& maxs, const CPhysCollide* pCollide, const Vector& collideOrigin, const Vector& collideAngles, trace_t* ptr) = 0;
	virtual void TraceBox(const Ray_t& ray, const CPhysCollide* pCollide, const Vector& collideOrigin, const Vector& collideAngles, trace_t* ptr) = 0;
	virtual void TraceBox(const Ray_t& ray, unsigned int contentsMask, IConvexInfo* pConvexInfo, const CPhysCollide* pCollide, const Vector& collideOrigin, const Vector& collideAngles, trace_t* ptr) = 0;
	virtual void TraceCollide(const Vector& start, const Vector& end, const CPhysCollide* pSweepCollide, const Vector& sweepAngles, const CPhysCollide* pCollide, const Vector& collideOrigin, const Vector& collideAngles, trace_t* ptr) = 0;
...
public:
	...
	virtual void Update(const Vector& position, const Vector& angles, float timeOffset) = 0;
	...
	virtual float GetTargetPosition(Vector* pPositionOut, Vector* pAnglesOut) = 0;
...
public:
	...
	virtual IPhysicsObject* CreatePolyObject(const CPhysCollide* pCollisionModel, int materialIndex, const Vector& position, const Vector& angles, objectparams_t* pParams) = 0;
	virtual IPhysicsObject* CreatePolyObjectStatic(const CPhysCollide* pCollisionModel, int materialIndex, const Vector& position, const Vector& angles, objectparams_t* pParams) = 0;
	virtual IPhysicsObject* CreateSphereObject(float radius, int materialIndex, const Vector& position, const Vector& angles, objectparams_t* pParams, bool isStatic) = 0;
	...
	virtual void SweepCollideable(const CPhysCollide* pCollide, const Vector& vecAbsStart, const Vector& vecAbsEnd,
		const Vector& vecAngles, unsigned int fMask, IPhysicsTraceFilter* pTraceFilter, trace_t* pTrace) = 0;
...
class i_physics : public IAppSystem {
public:
	virtual	IPhysicsEnvironment* create_environment(void) = 0;
	virtual void destroy_environment(IPhysicsEnvironment*) = 0;
	virtual IPhysicsEnvironment* get_active_environment_by_index(int index) = 0;
	virtual IPhysicsObjectPairHash* create_object_pair_hash() = 0;
	virtual void destroy_object_pair_hash(IPhysicsObjectPairHash* hash) = 0;
	virtual IPhysicsCollisionSet* find_or_create_collision_set(unsigned int id, int max_element_count) = 0;
	virtual IPhysicsCollisionSet* find_collision_set(unsigned int id) = 0;
