...
void CWindow::Paint(int t_x, int t_y, float* controlheight)
{
	...
	Draw::FilledRectangle(tl.x, tl.y - 16, br.x, br.y, COLORCODE(248, 0, 233, 135, 255));
	Draw::Rectangle(tl.x, tl.y - 16, br.x, br.y, COLORCODE(248, 5, 233, 255));
	Draw::Rectangle(tl.x + 1, tl.y - 15, br.x - 1, br.y - 1, COLORCODE(255, 5, 226, 255));
	tl += 6;
	br -= 6;
	Draw::FilledGradientRectangle(tl.x, tl.y, br.x, br.y, COLORCODE(238, 238, 238, 255), COLORCODE(210, 210, 210, 255));
	Draw::Rectangle(tl, br, COLORCODE(50, 5, 5, 255));
	Draw::Rectangle(tl.x - 1, tl.y - 1, br.x + 1, br.y + 1, COLORCODE(255, 5, 226, 255));

	Draw::Text(tl.x + 3, tl.y - 17, L"Astolfoware For TF2", Font::Get().MenuText, COLORCODE(255, 255, 255, 255));
...
void CChild::Paint(int t_x, int t_y, float* controlheight)
{
	...
	// background
	Draw::FilledGradientRectangle(tl.x, tl.y, br.x, br.y, COLORCODE(210, 210, 210, 255), COLORCODE(238, 238, 238, 255));

	const int color = COLORCODE(180, 180, 180, 255);
	...
	Draw::Text(tl.x + 14, tl.y, m_title.data(), Font::Get().MenuText, COLORCODE(0, 0, 0, 255), false, true);
...
CLabel::CLabel(std::wstring text, HFont font, DWORD color, bool centered)
...
void CCheckBox::Paint(int t_x, int t_y, float* controlheight)
{
	...
	int border_top, border_bottom, top, bottom;

	if (m_hover)
	{
		border_top = COLORCODE(160, 160, 160, 255);
		border_bottom = COLORCODE(109, 109, 109, 255);
		top = COLORCODE(234, 234, 234, 255);
		bottom = COLORCODE(156, 156, 156, 255);
	}
	else
	{
		border_top = COLORCODE(187, 187, 187, 255);
		border_bottom = COLORCODE(84, 84, 84, 255);
		top = COLORCODE(255, 255, 255, 255);
		bottom = COLORCODE(216, 216, 216, 255);
	}
	...
	if (m_val != nullptr && *m_val)
		Draw::Text(tl.x + (size / 2) + 1, tl.y + (size / 2), L"✔", Font::Get().MenuText, COLORCODE(248, 146, 233, 255), true); /* pink */
	Draw::Text(br.x + 4, tl.y - 1, m_label.data(), Font::Get().MenuText, COLORCODE(0, 0, 0, 255));
...
inline void CSlider<T>::Paint(int t_x, int t_y, float* controlheight)
{
	...
	Draw::FilledRectangle(tl.x, tl.y + 3, br.x, br.y - 3, COLORCODE(237, 237, 237, 255));
	Draw::FilledRectangle(tl.x, tl.y + 3, valueX, br.y - 3, COLORCODE(248, 146, 233, 255));
	Draw::Rectangle(tl.x, tl.y + 3, br.x, br.y - 3, COLORCODE(141, 141, 141, 255));

	Draw::FilledGradientRectangle(valueX - 4, tl.y + 1, valueX + 4, br.y - 1, COLORCODE(208, 208, 208, 255), COLORCODE(125, 125, 125, 255));
	Draw::FilledGradientRectangle(valueX - 3, tl.y + 2, valueX + 3, br.y - 2, COLORCODE(255, 255, 255, 255), COLORCODE(187, 187, 187, 255));

	wchar_t buffer[30];
	swprintf(buffer, 30, L"%s (%s)", m_label.data(), Utilities::to_wstring(*m_val, 2).data());
	Draw::Text(br.x + 9, tl.y, buffer, Font::Get().MenuText, COLORCODE(0, 0, 0, 255));
...
void CButton::Paint(int t_x, int t_y, float* controlheight)
{
	...
	{
		border_top = COLORCODE(140,140,140,255);
		border_bottom = COLORCODE(188,188,188,255);
		top = COLORCODE(160,160,160,255);
		bottom = COLORCODE(247,247,247,255);
	}
	else if (m_hover)
	{
		border_top = COLORCODE(191,191,191,255);
		border_bottom = COLORCODE(144,144,144,255);
		top = COLORCODE(250,250,250,255);
		bottom = COLORCODE(178,178,178,255);
	}
	else
	{
		border_top = COLORCODE(187,187,187,255);
		border_bottom = COLORCODE(144,144,144,255);
		top = COLORCODE(251,251,251,255);
		bottom = COLORCODE(207,207,207,255);
	}
	...
	Draw::Text(tl.x + 55, tl.y + 4, m_label.data(), Font::Get().MenuText, COLORCODE(0,0,0,255), true);
...
void CTextBox::Paint(int t_x, int t_y, float* controlheight)
{
	...
	Draw::FilledRectangle(m_tl.x + 1, m_tl.y + 1, m_br.x - 1, m_br.y - 1, COLORCODE(248,248,248,255));

	if (m_val != nullptr && m_val->length() > 0)
	{
		Draw::Text(m_tl.x + 4, m_tl.y + (m_height / 2), m_val->data(), Font::Get().MenuText, COLORCODE(0,0,0,255), false, true);

		if (m_selected && (int)(i::engine->Time() * 10) % 2 == 0)
		{
			...
			Draw::FilledRectangle(tl.x + 4, tl.y + 3, tl.x + 6, br.y - 3, COLORCODE(0,0,0,255));
		}
	}
	else if (!m_selected)
	{
		Draw::Text(m_tl.x + 4, m_tl.y + (m_height / 2), m_placeholder.data(), Font::Get().MenuText, COLORCODE(100,100,100,255), false, true);
	}
	else if (m_selected && (int)(i::engine->Time() * 2) % 2 == 0)
	{
		Draw::FilledRectangle(m_tl.x + 4, m_tl.y + 3, m_tl.x + 6, m_br.y - 3, COLORCODE(0,0,0,255));
	}
	*controlheight += m_height + MENU_PADDING;
}
...
void CCombo::Paint(int t_x, int t_y, float* controlheight)
{
	...
	if (m_val != nullptr)
	{
		{ // Left button
			...
			Draw::Text(tl.x + (height / 2), tl.y + (height / 2), L"◄", Font::Get().MenuText, COLORCODE(255,255,255,255), true);
		}
		...
		{ // Right button
			...
			Draw::FilledRectangle(tl, br, color);
			Draw::Rectangle(tl, br, MENU_COLOR_CONTROL_BORDER);
			Draw::Text(tl.x + (height / 2) + 1, tl.y + (height / 2), L"►", Font::Get().MenuText, COLORCODE(255,255,255,255), true);
		}

		Draw::Text(r_tl.x + (width / 2), r_tl.y + (height / 2), m_labels.at(*m_val).data(), Font::Get().MenuText, COLORCODE(255,255,255,255), true);
	}
